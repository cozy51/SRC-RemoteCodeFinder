...
import { deleteCard, imageUrlKey, loadCloudSettings, saveCommand, saveFavoriteChange, saveImageUrl, saveNote, type CommandOverrides, type Favorite, type ImageUrls, type NoteOverrides } from './services/favoritesStorage';
...
function App() {
  ...
  const [commands, setCommands] = useState<CommandOverrides>({});
  ...
  const [commandEditor, setCommandEditor] = useState<{ item: RemoteCode; value: string }>();
  ...
  const results = useMemo(() => {
    ...
    return remoteCodes
      .filter((item) => {
        ...
        const command = Object.hasOwn(commands, urlKey) ? commands[urlKey] ?? '' : item.command;
        return item.mode === mode && !deletedCards.includes(cardKey(item)) && (!normalized || `${item.code}\n${command}\n${item.note ?? ''}`.toLocaleLowerCase().includes(normalized));
      })
      .sort((a, b) => codeCollator.compare(a.code, b.code));
  }, [commands, deletedCards, mode, query]);
  ...
  async function saveEditedCommand() {
    ...
    try {
      const command = commandEditor.value.trim() || null;
      setCloudState('syncing'); setCloudMessage('Google Driveへ保存中…');
      setCommands(await saveCommand(commandEditor.item.mode, commandEditor.item.code, command));
      setCommandEditor(undefined); setCloudMessage('Google Driveと同期済み'); setCloudState('synced'); setToast(command ? 'コマンドを保存しました' : 'コマンドを空欄にしました');
    } catch (error) { setCloudMessage(error instanceof Error ? error.message : 'コマンドを保存できませんでした'); setCloudState('unsynced'); }
  }
  ...
  return <>
    ...
    <main>
      ...
      <div className="code-grid">{results.map((item, index) => {
        ...
        const storedCommand = commands[urlKey];
        const command = Object.hasOwn(commands, urlKey) ? storedCommand ?? '' : item.command;
        const displayItem = { ...item, command, imageUrl };
        return <article key={`${item.mode}-${item.code}-${index}`} ref={selected ? highlightedRef : undefined} className={`code-card ${selected ? 'highlighted' : ''}`}>
          <div className="code-top"><div className="code-identity"><span className="field-label">コード</span><button className="code-number" onClick={() => copyCode(item.code)} title="クリックしてコピー">{item.code}<small>タップしてコピー</small></button></div><div className="code-tools">{imageUrl && <button className="view-image" onClick={() => setImageItem(displayItem)}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 5.5h16v13H4zM7 15l3-3 2.5 2.5 2-2 2.5 2.5M8.5 9a1 1 0 1 0 0 .01" /></svg><span>説明画像を見る</span></button>}<button className={`star ${favoriteSet.has(favoriteKey(item)) ? 'selected' : ''}`} onClick={() => toggleFavorite(item)} aria-label={`${item.code}をお気に入り${favoriteSet.has(favoriteKey(item)) ? '解除' : '登録'}`}>{favoriteSet.has(favoriteKey(item)) ? '★' : '☆'}</button></div></div>
          {duplicateCodes.has(item.code) && <span className="duplicate-label">⚠ 同じコードが複数あります</span>}<div className="card-field command-field"><span className="field-label">コマンド</span><h3>{command || '（コマンド空欄）'}</h3><button className="note-edit" onClick={() => setCommandEditor({ item, value: command })}>コマンドを編集</button></div><div className="card-field note-field"><span className="field-label">備考</span>{note ? <p className="note">{note}</p> : <p className="note-empty">（備考なし）</p>}<button className="note-edit" onClick={() => setNoteEditor({ item, value: note ?? '' })}>{note ? '備考を編集' : '備考を追加'}</button></div>
          <div className="card-footer"><span>{item.mode}</span><div className="image-actions"><button onClick={() => prepareImageUrl(displayItem)}>{imageUrl ? '画像URLを変更' : '画像URLを登録'}</button><details className="more-menu"><summary aria-label={`${item.code}のその他の操作`}>•••</summary><div>{imageUrl && <button className="image-remove" onClick={() => removeImageUrl(item)}>画像を削除</button>}<button className="card-remove" onClick={() => removeCard(item)}>カードを削除</button></div></details></div></div>
        </article>;
      })}</div>
    ...
    {commandEditor && <div className="modal-backdrop" role="presentation"><section className="modal note-editor" role="dialog" aria-modal="true" aria-labelledby="command-editor-title"><p className="eyebrow">コマンドの編集</p><h2 id="command-editor-title">コード {commandEditor.item.code}</h2><label htmlFor="command-value">コマンド</label><textarea id="command-value" value={commandEditor.value} onChange={(event) => setCommandEditor({ ...commandEditor, value: event.target.value })} placeholder="コマンドを入力してください" autoFocus /><p className="editor-help">空欄で保存するとコマンドを空欄にできます。</p><div className="confirm-actions"><button onClick={() => setCommandEditor(undefined)}>キャンセル</button><button className="confirm-primary" onClick={saveEditedCommand}>保存</button></div></section></div>}
